...
import mealService, { MealDeleteRequestItem } from '../../services/mealService';
import { formatDate } from '../../utils/dateUtils';

interface PendingMealDeleteRequestsProps {
  onResponded?: () => void;
...
export const PendingMealDeleteRequests: React.FC<PendingMealDeleteRequestsProps> = ({ onResponded, onError }) => {
  const { theme } = useTheme();
  const [requests, setRequests] = useState<MealDeleteRequestItem[]>([]);
  ...
  const fetchRequests = useCallback(async (showLoading = true) => {
    ...
    try {
      const res = await mealService.getMyDeleteRequests();
      if (!mountedRef.current) return;
      if (res.success && Array.isArray(res.data)) {
        const list = [...res.data];
        setRequests(list.filter((r): r is MealDeleteRequestItem => !!r && r.status === 'pending'));
      } else {
        setRequests([]);
  ...
  const handleRespond = useCallback(async (requestId: string, action: 'accept' | 'reject') => {
    ...
    setRespondingId(id);
    try {
      const res = await mealService.respondToDeleteRequest(id, action);
      if (!mountedRef.current) return;
      if (res.success) {
  ...
  if (fetchError && requests.length === 0) {
    return (
      <View style={[styles.container, styles.errorContainer, { borderColor: theme.border?.secondary ?? theme.cardBorder ?? 'transparent' }]}>
        <ThemedText style={[styles.errorText, { color: theme.text?.secondary }]}>{fetchError}</ThemedText>
        <TouchableOpacity style={[styles.retryBtn, { backgroundColor: (theme.primary ?? theme.secondary) + '20' }]} onPress={handleRetry}>
          <Ionicons name="refresh" size={18} color={theme.primary ?? theme.secondary} />
          <ThemedText style={[styles.retryText, { color: theme.primary ?? theme.secondary }]}>Retry</ThemedText>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: (theme.status?.warning ?? theme.primary) + '12', borderColor }]}>
      ...
      <ThemedText style={[styles.subtitle, { color: theme.text?.secondary }]}>
        Admin requested to delete the following meal(s). Confirm or reject.
      </ThemedText>
      {requests.map((req) => {
        const id = req._id != null ? String(req._id) : '';
        if (!id) return null;
        const requestedByName = typeof req.requestedBy === 'object' && req.requestedBy && (req.requestedBy.name || req.requestedBy.email)
          ? (req.requestedBy.name || req.requestedBy.email)
          : 'Admin';
        const dateStr = safeFormatDate(req.mealDate);
        ...
        return (
          ...
          >
            <View style={styles.cardBody}>
              <ThemedText style={[styles.mealDate, { color: theme.text?.primary }]}>
                Meal for {dateStr || '—'}
              </ThemedText>
              <ThemedText style={[styles.mealSummary, { color: theme.text?.secondary }]} numberOfLines={1}>
                {req.mealSummary || 'Breakfast, Lunch, Dinner'}
              </ThemedText>
              ...
              >
                {isResponding ? (
                  ...
                  <>
                    ...
                    <ThemedText style={[styles.btnText, { color: theme.status?.error ?? '#ef4444' }]}>Reject</ThemedText>
                  </>
                )}
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                {isResponding ? (
                  ...
                  <>
                    ...
                    <ThemedText style={[styles.btnText, { color: theme.status?.success ?? '#10b981' }]}>Accept & delete</ThemedText>
                  </>
                )}
              </TouchableOpacity>
            </View>
