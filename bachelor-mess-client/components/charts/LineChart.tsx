...
export const LineChart: React.FC<LineChartProps> = ({
  ...
  const safeData = data || [];
  const allValues = safeData.flatMap(item => [item.value || 0, item.forecast || 0]);
  ...
  const trend = calculateTrend(safeData.map(item => item.value || 0));
  ...
  const getXPosition = (index: number) => {
    return padding + (index / (safeData.length - 1)) * (chartWidth - 2 * padding);
  };
          ...
          {safeData.length > 1 && (
            <View style={styles.lineContainer}>
              {safeData.map((item, index) => {
                ...
                const y1 = getYPosition(prevItem.value);
                const x2 = getXPosition(index);
                const y2 = getYPosition(item.value);
              ...
              <Pressable
                ...
                style={[
                  styles.point,
                  {
                    left: x - pointRadius,
                    top: y - pointRadius,
                    backgroundColor: selectedIndex === index ? chartColor : theme.surface,
