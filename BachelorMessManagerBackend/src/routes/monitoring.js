...
// @access  Private (Admin only)
router.get('/backups', protect, authorize('admin'), async (req, res, next) => {
  try {
...
// @access  Private (Admin only)
router.post('/backups', protect, authorize('admin'), async (req, res, next) => {
  try {
...
// @access  Private (Admin only)
router.post('/backups/:fileName/restore', protect, authorize('admin'), async (req, res, next) => {
  try {
...
// @access  Private (Admin only)
router.get('/backups/:fileName/verify', protect, authorize('admin'), async (req, res, next) => {
  try {
    const { fileName } = req.params;
...
// @access  Private (Admin only)
router.get('/logs', protect, authorize('admin'), (req, res, next) => {
  try {
...
// @access  Private (Admin only)
router.post('/metrics/reset', protect, authorize('admin'), (req, res, next) => {
  try {
...
// @access  Private (Admin only)
router.get('/status', protect, authorize('admin'), (req, res, next) => {
  try {
    const status = {
      timestamp: new Date().toISOString(),
      system: {
